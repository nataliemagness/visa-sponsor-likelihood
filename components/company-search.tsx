...
export function CompanySearch() {
  ...
  function navigate(slug: string) {
    ...
    router.push(role
      ? `/company/${slug}?role=${encodeURIComponent(role)}`
      : `/company/${slug}`)
  }
  ...
  return (
    <div ref={containerRef} className="relative w-full space-y-2.5">
      ...
      <input
        ...
        placeholder="Job title (optional) — e.g. Software Engineer, SDR, Data Analyst"
        className="w-full h-11 px-4 rounded-xl text-sm text-white placeholder-[#9ca3af] bg-[#1a1a1a] border border-[#2a2a2a] focus:outline-none focus:border-[#6C47FF]/50 focus:ring-1 focus:ring-[#6C47FF]/30 transition-colors"
      />
      {roleQuery.trim() && (
        <p className="text-xs text-[#9ca3af] pl-1">
          Select a company above to see how likely they are to sponsor a <span className="text-white">{roleQuery.trim()}</span> role.
        </p>
      )}
